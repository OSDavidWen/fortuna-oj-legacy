...
int main(int argc, char* argv[]){
        //argv[1] means test mode. 1 for file input/output, 2 for standard input/output, 3 for output submit task
        ...
        if (type <= 1){
                ...
                if (pid == 0){
                        ...
                        limit.rlim_cur = limit.rlim_max = 10485760;
                        ...
                        for (;;){
                                ...
                                if (WIFEXITED(runstat)){
                                        ...
                                        if (WTERMSIG(runstat) == SIGKILL)
                                                result(5, 0, time_usage, mem_usage, "SIGKILL");
                                        ...
                                        if (WTERMSIG(runstat) == SIGXFSZ){
                                                result(4, 0, 0, mem_usage, "SIGXFSZ");
                                        }
                                        ptrace(PTRACE_KILL, pid, NULL, NULL);
                                        break;
                        ...
                        if (spj){ //default
                                ...
                                char exec_diff[256];
                                sprintf(exec_diff, "diff --strip-trailing-cr --ignore-trailing-space %s %s >/dev/null 2>&1\n", argv[4], argv[5]);
                                //FILE *res = popen(exec_diff, "r");
                                //while (fscanf(res, "%[^\n]\n", msg) > 0) printf("%s", msg);
